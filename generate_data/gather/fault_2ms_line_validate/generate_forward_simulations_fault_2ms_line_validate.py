...
SIM_RUN = "fault_2ms_line_validate"
...
QC_FREQ = 100 # when to output full wavefield or not (I/O heavy)
n_processes = 4
...
if RANDOMSOURCE:# 3 sources per velocity model
    ...
    source_is = np.expand_dims(np.stack([(14+np.arange(0,120,20)), SOURCE_Yi*np.ones(6)]).T,0)
    source_is = np.repeat(source_is, N_VELS, axis=0)
